...
dtype2a = np.dtype([
    ('t', float),     # MJD
    ('flux', float),  # Jy
    ('err', float)    # optional measurement error in Jy
])
...
# define dtype: store spectrum as an array
dtype2b = np.dtype([('spectrum', float, (n_wave,))])

# create array
spectra = InfoArray(shape=(n_spec,), dtype=dtype2b, info={
    'wavelengths': np.linspace(500, 600, n_wave),  # nm
    't0': 59000.0,   # starting MJD
    'delta_t': 1.5   # time step in days
})

# populate example spectra
spectra[0]['spectrum'] = np.array([1.0, 1.1, 1.2, 1.1, 1.0])
spectra[1]['spectrum'] = np.array([1.05, 1.15, 1.25, 1.15, 1.05])
spectra[2]['spectrum'] = np.array([1.02, 1.12, 1.22, 1.12, 1.02])

print("2b:")
print(spectra)
print("Metadata:", spectra.info, "\n")
...
dtype2c = np.dtype([
    ('t', float),       # MJD of observation
    ('wavelength', object), # variable-length array
    ('spectrum', object)    # variable-length array
])

# create example array
irreg_spectra = np.array([
    (59000.1, np.array([500, 510, 520]), np.array([1.0, 1.1, 1.2])),
    (59001.7, np.array([505, 515, 525, 535]), np.array([0.9, 1.0, 1.05, 1.1])),
    (59003.3, np.array([500, 512, 524]), np.array([1.05, 1.15, 1.2]))
], dtype=dtype2c)
